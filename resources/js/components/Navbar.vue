    ...
    <div>
        <nav class="p-5 flex items-center justify-between">
            <router-link :to="{ name: 'home'}">
                <svg width="100" height="40" viewBox="0 0 46 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path id="M"
            ...
            <div v-else>
                <button type="button" @click="showMenu=!showMenu"
                    ...
                    <div class="flex items-center ml-8 opacity-100 hover:opacity-75 transition duration-150 ease-in-out">
                        ...
                        <img class="w-10 h-10 rounded-full ml-2" :src="user.profile_image || 'img/user-placeholder.png'" alt="user image">
                    </div>
        ...
        <div v-if="!authenticated"
             class="md:hidden flex-col content-end border rounded w-full max-w-md p-2 bg-transparent bg-opacity-75 shadow-inner float-right -mt-5 mr-5 transition duration-150 ease-in-out"
             :class="[showMenu ? 'flex' : 'hidden' ]">
            <router-link :to="{ name: 'login'}" class="py-2 hover:bg-gray-400 rounded focus:text-indigo-500"><nav-link>Login</nav-link></router-link>
            <router-link :to="{ name: 'register'}" class="py-2 hover:bg-gray-400 rounded focus:text-indigo-500"><nav-link>Sign up</nav-link></router-link>
